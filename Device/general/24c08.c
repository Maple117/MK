...
uint32_t Read24cxx(uint8_t id,uint32_t addr,u8 *p,uint32_t len)
{
	uint32_t i;
	IICStart();
	if(IICWriteByte(id+(( (addr) / 256 ) << 1 ))) return(1);
        if(IICWriteByte((addr) % 256)) return(1);      
	IICStart();
...
uint32_t Write24cxx(uint8_t id,uint32_t addr,u8 *p,uint32_t len)
{
	...
	{
	  IICStart();
          if(IICWriteByte(id+( ((addr + j) / 256 ) << 1 ))) return(1);
          if(IICWriteByte((addr + j) % 256)) return(1);               
